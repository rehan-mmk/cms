...
<head>
  ...
  <title> DMSGP | Parcels ready for collection </title>
            ...
            <div class="container-fluid">
                <div class="row mt-5">
                    ...
                    <div class="col-md-12">
                                        ...
                                        @foreach ($parcels as $parcel)
                                            ...
                                            <td>
                                              <select class="form-control form-control-sm" id="ParcelStatus" style="width: auto;">
                                                ...
                                                <option data-id="{{ $parcel->id }}" value="{{$status->id}}" @if ($status->id == $parcel->status->id) selected @endif>{{ __('parcel.' . $status->ParcelStatus) }}</option>
                                                @endforeach
                                              </select>
                                            </td>
                                            ...
                                            @if(Auth()->user()->role->view_permission == 1)
                                              <td style="padding-right: 0; padding-left: 0;">
                                                <a class="btn btn-primary btn-flat" href="{{ route('EditParcelView', $parcel->id) }}">
                        ...
                        <div class="row mt-3 pl-0 pl-md-4 pr-0 pr-md-4">
                          <div class="col-md-6 d-flex justify-content-center justify-content-md-start">
                              Showing {{ $parcels->firstItem() }} to {{ $parcels->lastItem() }} of {{ $parcels->total() }} entries
                          </div>
                          <div class="col-md-6 d-flex justify-content-center justify-content-md-end">
                              <ul class="pagination">
                                  <!-- Previous Page Link -->
                                  <li class="paginate_button page-item previous {{ $parcels->onFirstPage() ? 'disabled' : '' }}">
                                      <a class="page-link" href="{{ $parcels->previousPageUrl() }}">Previous</a>
                                  </li>
                                  ...
                                  @for ($page = $startPage; $page <= $endPage; $page++)
                                      <li class="paginate_button page-item {{ $page == $currentPage ? 'active' : '' }}">
                                          <a class="page-link" href="{{ $parcels->url($page) }}">{{ $page }}</a>
                                      </li>
                                  @endfor
                                  ...
                                  <!-- Next Page Link -->
                                  <li class="paginate_button page-item next {{ $parcels->hasMorePages() ? '' : 'disabled' }}">
              ...
              response.data.items.forEach(item => {
                ...
                $('.ItemsInParcel').append(`
                    <tr>
                        ...
                        <td>${additionalCharges !== 'null' ? additionalCharges + '$' : 'null'}</td>
                        <td>${item.amount}$</td>
                    </tr>
                `);
              });
